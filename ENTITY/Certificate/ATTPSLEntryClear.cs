namespace Entity.Certificate
{
    public class ATTPSLEntryClear
    {
        public string PSLNo { get; set; }
        public string ShareType { get; set; }
        public string ShOwnerTypeName { get; set; }
        public int? CertStatus { get; set; }

        public int? PledgeKitta { get; set; }
        public int? CertNo { get; set; }
        public string SelectedAction { get; set; }
        public string Status { get; set; }
        public string Name { get; set; }
        public int? SrNoFrom { get; set; }
        public int? SrNoTo { get; set; }
        public string compcode { get; set; }
        public int? ShholderNo { get; set; }
        public string Title { get; set; }
        public string FName { get; set; }
        public int? SeqNo { get; set; }
        public int? Seq_No { get; set; }
        public string LName { get; set; }
        public string Address { get; set; }
        public string FaName { get; set; }
        public string GrFaName { get; set; }

        public int? TotalKitta { get; set; }
        public int? ShKitta { get; set; }

        public string UserName { get; set; }
        public string Entrydate { get; set; }
        public string ActionType { get; set; }
        public string ShOwnerType { get; set; }
        public string ShownerSubtype { get; set; }

        public string Remarks { get; set; }

        public string PledgeAtList { get; set; }
        public string Remark { get; set; }
        public string TranDt { get; set; }

        public string Code { get; set; }
        public decimal PledgeAmount { get; set; }
        public string Transdate { get; set; }


        public string Pcode { get; set; }

        public string AppRemarks { get; set; }
        public string HolderLock { get; set; }
        public int? refAppNo { get; set; }
        // public string PledgeAmount { get; set; }

        public string BOID { get; set; }
        public string PledgeCenter { get; set; }

        public string TranType { get; set; }
        public int? PSL_clear_No { get; set; }
        public string ClearedDt { get; set; }
        public string Issuedup { get; set; }
        public string ChalaniNo { get; set; }
        public string ClearRemark { get; set; }
        public string Charge { get; set; }
        public string Clear_Charge { get; set; }
        public string IP { get; set; }



    }
}
