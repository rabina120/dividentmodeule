...
var AccountValidationViewModal = function () {
    ...
    self.LoadBatchList = function () {
        ...
        if (companyCode != '') {
            if (self.SelectedDividend() != null) {
                ...
                $.ajax({
                    type: "post",
                    url: '/ESewaTransaction/BatchProcessing/GetAllActiveBatch',
    ...
    self.CheckStatus = function () {
        ...
        if (self.CurrentBatch() != null) {
            ...
            $.ajax({
                type: 'POST',
                url: '/ESewaTransaction/BatchProcessing/CheckBatchStatus',
    ...
    self.CreateBatch = function () {
        if (self.Validation()) {
            ...
            $.ajax({
                type: 'POST',
                url: '/ESewaTransaction/BatchProcessing/CreateBatch',
                data: { 'CompCode': self.SelectedCompany(), 'DivCode': self.SelectedDividend()},
                dataType: 'json', beforeSend: function (xhr) {
                    xhr.setRequestHeader("XSRF-TOKEN",
                        $('input:hidden[name="__RequestVerificationToken"]').val());
    ...
    $('#fileupload').on('change', function () {
        ...
        if (goodFile) {
            ...
            $.ajax({
                type: "POST",
                url: "/ESewaTransaction/BatchProcessing/GetSheetNames?CompCode="+self.SelectedCompany()+"&DivCode="+self.SelectedDividend()+"&BatchID="+self.CurrentBatch(),
                async: true, beforeSend: function (xhr) {
                    xhr.setRequestHeader("XSRF-TOKEN",
                        $('input:hidden[name="__RequestVerificationToken"]').val());
                },
    ...
    self.GenerateDummyExcel = function () {
        window.location.href = "/ESewaTransaction/BatchProcessing/DownloadExcelDocument"
}
            ...
            if (self.ImportValidation()) {
                ...
                if (check == true) {
                    ...
                    $.ajax({
                        ...
                        url: "/ESewaTransaction/BatchProcessing/UploadSheet?SheetId=" + self.SelectedSheet().SheetID() + "&CompCode="+self.SelectedCompany()+"&DivCode="+self.SelectedDividend()+"&BatchID="+self.CurrentBatch(),
                        async: true,
                        beforeSend: function (xhr) {
                            xhr.setRequestHeader("XSRF-TOKEN",
                                $('input:hidden[name="__RequestVerificationToken"]').val());
    ...
    self.ValidateCDSData = function () {
        if (self.Validation()) {
            ...
            $.ajax({
                ...
                url: '/ESewaTransaction/BatchProcessing/ValidateCDSData',
                data: { 'CompCode': self.SelectedCompany(), 'DivCode': self.SelectedDividend(),'BatchID':self.CurrentBatch() },
                dataType: 'json',
                success: (result) => {
                    if (result.isSuccess) {
        ...
        if (self.Validation()) {
            if (!Validate.empty(self.CurrentBatch())) {
                ...
                $.ajax({
                    ...
                    url: '/ESewaTransaction/BatchProcessing/UpdateBankDetails',
                    data: { 'CompCode': self.SelectedCompany(), 'DivCode': self.SelectedDividend(), 'BatchID': self.CurrentBatch() },
                    dataType: 'json',
                    success: (result) => {
                        if (result.isSuccess) {
        ...
        if (self.Validation() ) {
            if (!Validate.empty(self.CurrentBatch())) {
                ...
                $.ajax({
                    ...
                    url: '/ESewaTransaction/BatchProcessing/ValidateAccountDetails',
    ...
    self.RouteToTransaction = function () {
        window.location.href ="/eSewaTransaction/EsewaTransactionProcessing"
    }
    self.RouteToStatus = function () {
        window.location.href ="/eSewaTransaction/TransactionStatus"
    }
    ...
    self.enableDiv = function (data,isPartial,isValid) {
        ...
        if (data == "AV" || data == "TP") {
            ...
            $('#TransactionProcessingStatusTable').dataTable({
                ...
                buttons: ['excel','pdf'],
                "ajax": {
                    "url": "/ESewaTransaction/BatchProcessing/GetData",
                    "type": "POST", beforeSend: function (xhr) {
                        xhr.setRequestHeader("XSRF-TOKEN",
                            $('input:hidden[name="__RequestVerificationToken"]').val());
                    },
