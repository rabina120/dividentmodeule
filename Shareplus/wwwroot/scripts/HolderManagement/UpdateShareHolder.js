...
function CertDet(data, vm) {
    ...
    if (data != undefined) {
        ...
        self.share_type = ko.observable(data.share_type);
        ...
        //swirch case for cert status
        switch (data.certStatus) {
            case 1:
                self.CertStatusText = ko.observable("Normal");
                break;
            case 2:
                self.CertStatusText = ko.observable("Pledge");
                break;
            case 3:
                self.CertStatusText = ko.observable("Suspend");
                break;
            case 4:
                self.CertStatusText = ko.observable("Lost");
                break;
            case 5:
                self.CertStatusText = ko.observable("Dakhil Registered");
                break;
        }
...
var ManageShareHolder = function () {
    ...
    self.Validation = function () {
        ...
        //if (Validate.empty(self.Address1())) {
        //    errMsg += "Please Enter Address1<br/>"
        //}
        //if (Validate.empty(self.Address2())) {
        //    errMsg += "Please Enter Address2<br/>"
        //}
        ...
        //    errMsg += "Please Enter Tel No<br/>"
        //}
        //if (Validate.empty(self.MobileNo())) {
        //    errMsg += "Please Enter Mobile No<br/>"
        //}
        //if (Validate.email(self.EmailAdd())) {
        //    errMsg += "Please Enter Valid Email Address<br/>"
        //}
        ...
        //    errMsg += "Please Enter Bank Name<br/>";
        //}
        //if (Validate.empty(self.AccountNo())) {
        //    errMsg += "Please Enter Account No<br/>";
        //}
        if (optionAUD == "U") {
    ...
    self.LoadShOwnerSubType = function (a) {
        $.ajax({
            type: "post",
    ...
    self.LoadOccupation = function (a) {
        $.ajax({
            type: "post",
    ...
    self.LoadDistrict();
    self.GetApplicationNo = function (data) {
        if (self.ValidateCompany()) {
            if (!Validate.empty(self.ApplicationNo())) {
                Openloader()
                $.ajax({
                    type: "post",
                    url: '/HolderManagement/UpdateShHolder/GetApplicationInformation',
                    data: { 'ApplicationNo': data, 'CompCode': self.SelectedCompany() },
                    datatype: "json", beforeSend: function (xhr) {
                        xhr.setRequestHeader("XSRF-TOKEN",
                            $('input:hidden[name="__RequestVerificationToken"]').val());
                    },
                    success: function (result) {
                        ...
                        if (result.isSuccess) {
                            ...
                            GetSholderInformation(self.ShholderNo())
                        } else {
                            $('#ApplicationNo').attr('disabled', false)
                            alert('warning', result.message)
                            Closeloader()
                        }
                    },
    ...
    self.GetSholderInformation = function (data) {
        if (self.ValidateCompany()) {
            ...
            if (shholderNo != undefined && shholderNo != "") {
                ...
                $.ajax({
                    type: "post",
                    url: '/HolderManagement/UpdateShHolder/GetHolderInformation',
                    ...
                    success: function (result) {
                        ...
                        if (result.isSuccess) {
                            ...
                            self.LoadOccupation(result.responseData.occupation);
    ...
    self.SaveShHolder = function () {
        if (self.Validation()) {
                ...
                swal({
                    title: "Are you sure?",
                    text: "Holders Detail Will Be Updated.",
                    icon: "warning",
                    buttons: true,
                    dangerMode: true
                }).then((willSave) => {
                    if (willSave) {
                        ...
                        $.ajax
                            ({
                                type: "post",
                                url: "/HolderManagement/UpdateShHolder/SaveShholder",
                                data: {
