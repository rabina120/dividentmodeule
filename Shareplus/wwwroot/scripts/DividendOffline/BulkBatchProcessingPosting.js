...
var BulkIssue = function () {
    ...
    self.isIssue = ko.observable(true);
    self.isPay = ko.observable(true);
    self.SheetLists = ko.observableArray([]);
    self.SelectedSheet = ko.observable();


    self.TotalUploadedRecords = ko.observable();
    self.TotalWarrentAmt = ko.observable();
    self.TotalNetAmount = ko.observable();
    self.TotalTaxAmount = ko.observable();
    ...
    self.LoadDividendLists = function () {
        if (self.ValidateCompany()) {
            ...
            $.ajax({
                type: "post",
                url: '/DividentOffine/BulkBatchProcessingPosting/GetAllDividends',
    ...
    self.DummyDataDownload = function () {
        if (Validate.empty(self.DivBasedOn())) {
            ...
            window.location.href = "/DividentOffine/BulkBatchProcessingPosting/DownloadExcelDocument?DivBasedOn=" + self.DivBasedOn()
        }
    }
    ...
    $('#fileupload').on('change', function () {
        ...
        $.ajax({
            type: "POST",
            url: "/DividentOffine/BulkBatchProcessingPosting/GetSheetNames",
    ...
    self.UploadExcel = function () {
        ...
        const fileUpload = $("#fileupload").get(0);
        ...
        swal({
            ...
            if (willSave) {
                ...
                $.ajax({
                    type: "POST",
                    url: "/DividentOffine/BulkBatchProcessingPosting/UploadSheet?SheetId=" + ko.toJS(self.SelectedSheet().SheetId) + "&DivType=" + ko.toJS(self.DivBasedOn()),
                    beforeSend: function (xhr) {
                    ...
                    success: function (response) {
                        if (response.isSuccess) {
                            ...
                            self.TotalWarrentAmt(response.responseData);
                            self.TotalTaxAmount(response.taxAmount);
                            self.TotalNetAmount(response.netAmount);
                        } else {
                            // Show error message
                            swal({
                                title: "Error",
    ...
    self.PostPayment = function (data) {
        if (self.Validation()) {
            Openloader()
            $.ajax({
                ...
                url: "/DividentOffine/BulkBatchProcessingPosting/BulkDividendPayment",
                data: {
