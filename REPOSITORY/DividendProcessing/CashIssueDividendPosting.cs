...
{
    public class CashIssueDividendPosting : ICashIssueDividendPosting
    {
        IOptions<ReadConfig> connectionString;

        public CashIssueDividendPosting(IOptions<ReadConfig> _connectionString)
        {
            this.connectionString = _connectionString;
        }
        public JsonResponse GetCashDividendForApproval(string CompCode, string FromDate, string ToDate, string Divcode, string UserName, string IP)
        {
            ...
            using (SqlConnection connection = new SqlConnection(Crypto.Decrypt(connectionString.Value.DefaultConnection)))
            {
                try
                {
                    ...
                    if (tableName != null)
                    {
                        ...
                        List<ATTCashDividend> aTTCashDividend = SqlMapper.Query<ATTCashDividend, ATTShHolder, ATTCashDividend>(connection, sql: "Get_DividendIssue_ForApproval",
        ...
        public JsonResponse PostCashDividendRequest(List<ATTCashDividend> aTTCashDividends, ATTCashDividend RecordDetails, string ActionType, string UserName, string IP)
        {
            JsonResponse jsonResponse = new JsonResponse();
                ...
                try
                {
                    ...
                    using (SqlTransaction trans = connection.BeginTransaction())
                    {
                        ...
                        if (tableName != null)
                        {
                            ...
                            aTTCashDividends.ForEach(x => dt.Rows.Add(x.ShHolderNo, x.WarrantNo));

                            SqlCommand cmd = new SqlCommand("Cash_Dividend_Entry_Posting", connection);
